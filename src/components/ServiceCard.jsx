const ServiceCard = ({ service, variant = "full" }) => {
  ...
  return (
    <div className={`group bg-white dark:bg-[#3b4460] rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 border border-transparent hover:border-brand/10 overflow-hidden 
      ${isCompact ? 'p-4' : 'p-6 md:p-8 mx-6'}
      ${isHorizontal ? 'lg:flex lg:items-start lg:gap-6' : ''}
    `}>

      {/* Image Section */}
      <div className={`w-full h-48 overflow-hidden rounded-2xl
        ${isHorizontal ? 'lg:w-1/2 lg:h-full lg:max-h-72' : ''}
      `}>
        <img
          src={image}
          alt={name}
